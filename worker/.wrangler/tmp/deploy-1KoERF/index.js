...
async function generateVerificationLink(accessToken, email, env) {
  const res = await fetch(`${SEND_OOB_URL}?key=${env.WEB_API_KEY}`, {
    method: "POST",
    headers: {
  ...
  if (!res.ok || !raw.includes("oobCode")) {
...
__name(appendBytes, "appendBytes");
...
async function sendEmail(env, to, subject, html, text) {
  ...
  const send = /* @__PURE__ */ __name(async (line) => {
    await writer.write(TEXT.encode(`${line}\r
`));
    const r = await readResponse(reader, buffer);
    buffer = r.buffer;
    return r;
  }, "send");
  const expect = /* @__PURE__ */ __name((r, wanted, what) => {
  ...
  try {
    ...
    await writer.write(TEXT.encode(`${message}\r
.\r
`));
...
var index_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      const link = await generateVerificationLink(accessToken, payload.email, env);
      await sendEmail(
        env,
        payload.email,
        "BRINGIT \u25B6 VERIFY YOUR EMAIL",
        createVerificationEmailHtml({ email: payload.email, link }),
        createVerificationEmailText({ email: payload.email, link })
      );
